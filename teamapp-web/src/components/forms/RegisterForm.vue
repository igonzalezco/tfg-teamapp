  ...
  >
    ...
    <el-form-item :label="$t('register.passwordRepeat')" prop="confirmPassword">
      <el-input
        ...
        type="password"
        :placeholder="$t('register.placeholder.passwordRepeat')"
        show-password
      />
    </el-form-item>

    <el-form-item>
      <el-button :disabled="disableRegistrar" type="success" @click="submitRegister">{{
        $t('register.register')
      }}</el-button>
    </el-form-item>
  </el-form>
      ...
      const validatePassword = (rule, value, callback) => {
        if (!value) {
          ...
          if (!strongRegex.test(value)) {
            callback(new Error(this.$t('register.rules.passwordFormat')))
          } else {
            callback()
          }
        }
